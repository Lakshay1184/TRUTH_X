...
export default function ResultsDashboard({ tab, onReset, fileName, textSnippet, apiResult }: Props) {
  ...
  // Build real insights from ML results
  const buildInsights = () => {
    if (!apiResult) return mock.insights;
    const ins: string[] = [];
    if (apiResult.video_result) {
      const vr = apiResult.video_result;
      if (vr.label?.toLowerCase() === "fake") {
        ins.push(`Deepfake detection model identified this video as ${vr.label} with ${(vr.confidence * 100).toFixed(1)}% confidence.`);
      } else {
        ins.push(`Video frames appear authentic according to deepfake analysis (${(vr.confidence * 100).toFixed(1)}% confidence).`);
      }
    }
    if (apiResult.text_result) {
      const tr = apiResult.text_result;
      ins.push(`Text analysis: ${tr.label} (${(tr.ai_probability * 100).toFixed(1)}% AI probability, ${(tr.confidence * 100).toFixed(1)}% confidence).`);
    }
    if (apiResult.fact_check && apiResult.fact_check.length > 0) {
      ins.push(`Found ${apiResult.fact_check.length} related articles via fact-check search.`);
    }
    return ins.length > 0 ? ins : mock.insights;
  };
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="grid grid-cols-3 gap-4 mt-8">
                ...
                <div className="bg-black/20 rounded-xl p-3 border border-white/5">
                  ...
                  <div className="text-xl font-bold text-white">{apiResult?.processing_time_seconds ? `${apiResult.processing_time_seconds}s` : "2.4s"}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <motion.div variants={itemVars} className="space-y-6">
          ...
          <div className="glass-card rounded-3xl p-6">
            ...
            <div className="space-y-4">
              {(apiResult?.ai_models && apiResult.ai_models.length > 0
                ? apiResult.ai_models
                : [
                  { name: "FaceForensics++", score: 94, label: "Fake" },
                  { name: "DeepFakeDetector", score: 91, label: "Fake" },
                  { name: "AudioDeepDetect", score: 88, label: "Fake" }
                ]
              ).map((model: any, i: number) => {
                const isFake = model.label?.toLowerCase() === "fake";
                  ...
                  <div key={i} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className={`font-mono ${textColor}`}>
                        {model.label ? `${model.label}: ` : ""}{model.score}%
                      </span>
                    </div>
